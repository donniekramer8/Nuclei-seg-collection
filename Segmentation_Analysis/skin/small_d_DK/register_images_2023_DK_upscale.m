function register_images_2023_DK_upscale(pth,IHC,E,zc,szz,sk,tpout,regE)
    if nargin == 0
        pth = '\\fatherserverdw\andreex\students\Donald Monkey fetus\test_liver_cube\10x\';
    end
...
if ~exist('regE','var')
    regE.szE=301; % size of registration tiles % 250
    regE.bfE=200; % size of buffer on registration tiles 100
    regE.diE=500; % distance between tiles     % 150  200
end
...
for kk=1:length(mv)
    ...
    if exist([matpth,'D\',nm,'mat'],'file') && rc==0
        ...
        immvGg=register_global_im(immv,tform,cent,f,mode(immvg(:)));
        imwrite(immvGg,[outpthG,nm,tpout]);
        imwrite(immvGg,[pthclassified,nm,tpout]);
...
%         if IHC==1;ct=0.8;else;ct=0.92;end
        if IHC==1;ct=0.92;else;ct=0.91;end  % CHANGED ct = 0.94 from 0.9 - DONALD 7_7_23
