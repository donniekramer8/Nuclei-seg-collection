function test_invert_registration_DK(pth,IHC,E,zc,szz,sk,tpout,regE)
    if nargin == 0
        pth = '\\fatherserverdw\andre\Alzherimers brain\Fully Sectioned\FXFAD12FB2\2_5x_python\';
    end

invert_image_outpth='\\fatherserverdw\andre\Alzherimers brain\Fully Sectioned\FXFAD12FB2\2_5x_python\test_inverted\';
if ~exist('invert_image_outpth','dir');mkdir(invert_image_outpth); end
...
if ~exist('regE','var')
    regE.szE=351; % size of registration tiles % 250
    regE.bfE=100; % size of buffer on registration tiles 100
    regE.diE=100; % distance between tiles     % 150
end
...
padall=300; % padding around all images 10
...
if E;mkdir(outpthE);mkdir([matpth,'inverse_affine_matrix\']);end  % added by donnie
inv_matrix_path=[matpth,'inverse_affine_matrix\'];
...
for kk=1:length(mv)
    ...
    redo=0;
    if exist([matpth,'D\',nm,'mat'],'file') && redo==0
...
%         if IHC==1;ct=0.8;else;ct=0.92;end
        if IHC==1;ct=0.92;else;ct=0.9;end
        % try registration pairs 1
        try
            [immvGg,tform,inv_tform,cent,f,R]=calculate_global_reg_TEST_DK(imrfgA,immvg,rsc,iternum,IHC);
        catch
            disp('deu raia')
        end
        % try with registration pairs 2
        if R<ct;[immvGgB,tformB,inv_tformB,centB,fB,RB]=calculate_global_reg_TEST_DK(imrfgB,immvg,rsc,iternum,IHC);disp('RB');end % R<0.93
        % try with registration pairs 3
        if R<ct && RB<ct;[immvGgC,tformC,inv_tformC,centC,fC,RC]=calculate_global_reg_TEST_DK(imrfgC,immvg,rsc,iternum,IHC);disp('RC');end %R<0.93
...
%         figure(17);imshowpair(imrfg,immvG),title(R)
        TAmvG=register_global_im(TAmv,tform,cent,f,0);
        imwrite(immvG,[outpthG,nm,tpout]);

        % ADDED BY DONNIE
        imm_inverted=register_global_im(immvG,inv_tform,cent,f,fillval);
%         figure(17);imshowpair(imrfg,immvG(end:-1:1,:,:)),title(R)
%         figure(17);imshowpair(imrfg,immvG),title(R)
        TA_inv=register_global_im(TAmv,inv_tform,cent,f,0);
        imwrite(imm_inverted,[invert_image_outpth,nm,tpout]);
        nm_og = [nm(1:end-1), '_og.'];
        imwrite(immv,[invert_image_outpth,nm_og,tpout]);

        % save inverted affine matrix variable to mat file
        matrix = inv_tform.T;
        save([inv_matrix_path,imlist(krf).name(1:end-3),'mat'],"matrix");
