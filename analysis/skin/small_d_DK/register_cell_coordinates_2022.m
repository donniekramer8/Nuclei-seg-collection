function register_cell_coordinates_2022(pth0,pthcoords,scale,redo)
...
for kk=1:length(matlist)
    ...
    if exist([outpth,matlist(kk).name],'file') && ~redo;continue;end
    %if exist([outpth,matlist(kk).name],'file');disp('calculating again');end
    load([pthcoords,matlist(kk).name],'xy');
    %if isempty(xy) || ~isfile([pth0,matlist(kk).name(1:end-4),tp]);continue;end
    xy=xy/scale;
...
%     end
    ...
    xy=xy+padall;
    if pad2==1
        %a=imfinfo([pth0,matlist(kk).name(1:end-4),tp]);a=[a.Height a.Width];
