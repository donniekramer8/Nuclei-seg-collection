function save_images_elastic_scale_DK(pthim,pthdata,scale,ds,padnum,cropim,redo)
    if nargin == 0
        % pthim = '\\fatherserverdw\andre\monkey_extra\kidney10x\Hchannel\';
        % pthdata = '\\fatherserverdw\andre\monkey_extra\kidney2_5\registered_BTC\elastic registration\save_warps\';
        pthim = '\\fatherserverdw\andre\Alzherimers brain\Fully Sectioned\FXFAD12FB2\10x_python_new\';
        pthdata = '\\fatherserverdw\andre\Alzherimers brain\Fully Sectioned\FXFAD12FB2\1x_python_new\registered_BTC\elastic registration\save_warps\';
        scale=8;
        ds=1;
        cropim=1;
    end

% pthclassified = '\\fatherserverdw\andreex\students\Donald Monkey fetus\test_liver_cube\classified\';
...
outpth=[pthim,'registeredE_scale\'];
...
for kz=1:length(matlist)
    ...
    a=imfinfo([pthim,imnm]);a=[a.Height a.Width];
    IM=imread([pthim,imlist(kz).name],'PixelRegion',{[1 ds a(1)],[1 ds a(2)]});
