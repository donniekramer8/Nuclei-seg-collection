function register_images_2023_DK_mousebrain_rev(pth,IHC,E,zc,szz,sk,tpout,regE)
    if nargin == 0
        pth = '\\fatherserverdw\andre\Alzherimers brain\Fully Sectioned\FXFAD12FB2\1x_python_new\';
    end
...
if ~exist('regE','var')
    regE.szE=250; % size of registration tiles % 250
...
%         if IHC==1;ct=0.8;else;ct=0.92;end
        if IHC==1;ct=0.91;else;ct=0.94;end  % CHANGED ct = 0.94 from 0.9 - DONALD 7_7_23
        ...
        if R<ct;[immvGgB,tformB,centB,RB]=calculate_global_reg_DK_new(imrfgB,immvg,rsc,iternum,IHC);disp('RB');end % R<0.93
        ...
        save([matpth,nm,'mat'],'tform','cent','szz','padall','regE');
